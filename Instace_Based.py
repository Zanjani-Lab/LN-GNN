...
while n_prediction<npredictioncount:
    ...
    for i in range(Ntotal):
        if (numneigh_of_i[i] == maxneighcount) and ((timestepcount%stfreq)>-1):
            ...
            #perform k-nearest neighbor analysis for Instance-based prediction
            myk=10
            disttodata=np.zeros((len(mydata)),dtype=float)
            #print("shape of dist array= ", np.shape(disttodata))
            for cii,data0 in enumerate(mydata):
                for jj in range(maxneighcount):
                    disttodata[cii] += magnitudesq(np.array(data0)[jj]-myx[jj])
                    
            k_neighs_allindex=np.argsort(disttodata)
            sorted_distance=np.sort(disttodata)
            predicted_fx=0.0
            predicted_fy=0.0
            predicted_fz=0.0
            ndistzero=0
            predicted_fx_0dmatch=0.0
            predicted_fy_0dmatch=0.0
            predicted_fz_0dmatch=0.0
            predicted_fx_final=0.0
            predicted_fy_final=0.0
            predicted_fz_final=0.0
            mytotalw=0.0
            #print("shape of dist array sorted= ", np.shape(sorted_distance))
            #print("shape of dist array index sorted= ", np.shape(k_neighs_allindex))
            for kk in range(mykvalue):
                #find zero distances (exact matches)
                if sorted_distance[kk] < 0.0001:
                    ndistzero += 1
                    myindi=k_neighs_allindex[kk]
                    #print("shape of outdata= ", np.shape(mydata_Out))
                    #print("myindi is=", myindi)
                    predicted_fx_0dmatch = predicted_fx_0dmatch + mydata_Out[myindi][0]
                    predicted_fy_0dmatch = predicted_fy_0dmatch + mydata_Out[myindi][1]
                    predicted_fz_0dmatch = predicted_fz_0dmatch + mydata_Out[myindi][2]
                else:
                    wi=1.0/(sorted_distance[kk]*sorted_distance[kk])
                    myindi=k_neighs_allindex[kk]
                    #print("shape of outdata= ", np.shape(mydata_Out))
                    #print("myindi is=", myindi)
                    predicted_fx = predicted_fx + wi*mydata_Out[myindi][0]
                    predicted_fy = predicted_fy + wi*mydata_Out[myindi][1]
                    predicted_fz = predicted_fz + wi*mydata_Out[myindi][2]
                    mytotalw += wi
            if ndistzero > 0:
                    predicted_fx_final = predicted_fx_0dmatch / float(ndistzero)
                    predicted_fy_final = predicted_fy_0dmatch / float(ndistzero)
                    predicted_fz_final = predicted_fz_0dmatch / float(ndistzero)
            else:
                predicted_fx_final= predicted_fx/mytotalw
                predicted_fy_final= predicted_fy/mytotalw
                predicted_fz_final= predicted_fz/mytotalw
            if (n_prediction<npredictioncount): 
                n_prediction += 1
                mae_fx += abs((frx[i]+normF)/(2.00*normF)-predicted_fx_final)
                mae_fy += abs((fry[i]+normF)/(2.00*normF)-predicted_fy_final)
                mae_fz += abs((frz[i]+normF)/(2.00*normF)-predicted_fz_final)
                #f2x.write("%f %f \r\n" % ((frx[i]+normF)/(2.00*normF), predicted_fx_final)
                #f2y.write("%f %f \r\n" % ((fry[i]+normF)/(2.00*normF), predicted_fy_final)
                #f2z.write("%f %f \r\n" % ((frz[i]+normF)/(2.00*normF), predicted_fz_final)
    line000=f.readline()
    if not line000:
        break
f.close() 
print("number of neighbors== ", maxneighcount)
